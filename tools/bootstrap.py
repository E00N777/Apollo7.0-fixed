...
def set_cc_cxx_path(environ_cp):
    path_set = False
    if environ_cp.get('PATH'):
        env_paths = environ_cp.get('PATH').split(':')
        for p in env_paths:
            clang_path = '%s/clang' % p
            clangxx_path = '%s/clang++' % p
            if os.path.islink(clang_path):
                write_action_env_to_bazelrc('CC', os.path.realpath(clang_path))
                write_action_env_to_bazelrc('CXX', os.path.realpath(clangxx_path))
                path_set = True
                print("Found clang in %s" % p)
                break
    if not path_set:
        raise ValueError('Fail to locate clang and clang++')
